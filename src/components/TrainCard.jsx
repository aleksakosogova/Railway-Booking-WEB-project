...
const TrainCard = ({ train }) => {
  return (
    <div style={{ backgroundColor: '#ffffff', border: '1px solid #e1bee7', padding: '25px', borderRadius: '20px', marginBottom: '20px', boxShadow: '0 4px 15px rgba(74, 20, 140, 0.05)' }}>
      ...
      {/* Перехід на сторінку бронювання за ID потяга */}
      <Link to={`/booking/${train.id}`} style={{ textDecoration: 'none' }}>
        <button style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', backgroundColor: '#7b1fa2', color: 'white', fontWeight: 'bold', cursor: 'pointer', fontSize: '1rem' }}>
          Вибрати місця
        </button>
      </Link>
    </div>
